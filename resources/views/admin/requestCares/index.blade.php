...
@can('request_care_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.request-cares.create') }}">
                        ...
                        <tr data-entry-id="{{ $requestCare->id }}">
                            ...
                            <td>
                                @can('request_care_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.request-cares.show', $requestCare->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('request_care_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.request-cares.edit', $requestCare->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('request_care_delete')
                                    <form action="{{ route('admin.request-cares.destroy', $requestCare->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('request_care_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.request-cares.massDestroy') }}",
