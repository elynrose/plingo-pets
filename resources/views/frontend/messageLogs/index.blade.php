        ...
        <div class="col-md-12">
            @can('message_log_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.message-logs.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $messageLog->id }}">
                                        ...
                                        <td>
                                            @can('message_log_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.message-logs.show', $messageLog->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('message_log_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.message-logs.edit', $messageLog->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('message_log_delete')
                                                <form action="{{ route('frontend.message-logs.destroy', $messageLog->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('message_log_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.message-logs.massDestroy') }}",
